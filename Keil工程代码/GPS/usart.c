...
void USART3_IRQHandler(void)                	//串口1中断服务程序
{
	...
	if(Res == '$')//如果首位是这个，就开始读取，是开始位
	{
		point1 = 0;	
	}
		

	  USART_RX_BUF[point1++] = Res;//获取到的帧数据存放在数组中
	...
	if(USART_RX_BUF[0] == '$' && USART_RX_BUF[4] == 'M' && USART_RX_BUF[5] == 'C')			//确定是否收到"GPRMC/GNRMC"这一帧数据
	{
		if(Res == '\n')									   //如果收到了
		{
			...
			memcpy(Save_Data.GPS_Buffer, USART_RX_BUF, point1); 	//保存数据，把帧数据存放在数组里面
...
void clrStruct()
{
	...
	memset(Save_Data.Speed, 0, Speed_Length);
	memset(Save_Data.Direct, 0, Direct_Length);
